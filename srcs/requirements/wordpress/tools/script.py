...
try:
    ...
    subprocess.run(f"chmod 775 wp-content/uploads && chmod 775 wp-content/plugins && chmod 775 wp-content/themes", shell=True, check=False)
    subprocess.run("wp plugin install redis-cache --activate --path=/var/www/html --allow-root", shell=True, check=True)
    subprocess.run("wp plugin update --all --path=/var/www/html --allow-root", shell=True, check=False)
    subprocess.run("wp redis enable --path=/var/www/html  --allow-root", shell=True, check=True)
